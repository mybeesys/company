...
@section('content')
@php
    $canTrialBalance = dashboard_can(\Modules\Accounting\Support\AccountingPermissions::TRIAL_BALANCE_SHOW);
    $canBalanceSheet = dashboard_can(\Modules\Accounting\Support\AccountingPermissions::BALANCE_SHEET_SHOW);
    $canReceivablesAging = dashboard_can(\Modules\Accounting\Support\AccountingPermissions::RECEIVABLES_AGING_SHOW);
    $canPayablesAge = dashboard_can(\Modules\Accounting\Support\AccountingPermissions::PAYABLES_AGING_SHOW);
    $canJournalLedger = dashboard_can(\Modules\Accounting\Support\AccountingPermissions::JOURNAL_LEDGER_SHOW);
    $months = [];
    $debit_trend = [];
    $credit_trend = [];
    for ($i = 1; $i <= 12; $i++) {
...
<div class="container-fluid py-3">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">@if($canTrialBalance)<a href="{{ route('trial-balance', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي كل الحركات المدينة ضمن الفترة المحددة.">@else<div class="fin-kpi" data-bs-toggle="tooltip" title="إجمالي كل الحركات المدينة ضمن الفترة المحددة.">@endif<div class="text-muted">@lang('accounting::lang.total_debit')</div><div class="fw-bold fs-4">@format_currency($totals->total_debit ?? 0)</div>@if($canTrialBalance)</a>@else</div>@endif</div>
        <div class="col-md-3">@if($canTrialBalance)<a href="{{ route('trial-balance', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي كل الحركات الدائنة ضمن الفترة المحددة.">@else<div class="fin-kpi" data-bs-toggle="tooltip" title="إجمالي كل الحركات الدائنة ضمن الفترة المحددة.">@endif<div class="text-muted">@lang('accounting::lang.total_credit')</div><div class="fw-bold fs-4">@format_currency($totals->total_credit ?? 0)</div>@if($canTrialBalance)</a>@else</div>@endif</div>
        <div class="col-md-3">@if($canTrialBalance)<a href="{{ route('trial-balance', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="صافي الرصيد الدفتري للحركات بعد تطبيق الفلاتر.">@else<div class="fin-kpi" data-bs-toggle="tooltip" title="صافي الرصيد الدفتري للحركات بعد تطبيق الفلاتر.">@endif<div class="text-muted">@lang('accounting::lang.total_balance')</div><div class="fw-bold fs-4">@format_currency($total_balance ?? 0)</div>@if($canTrialBalance)</a>@else</div>@endif</div>
        <div class="col-md-3">@if($canTrialBalance)<a href="{{ route('trial-balance', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="صافي الحركة = الدائن - المدين؛ يعطي اتجاه السيولة الدفترية.">@else<div class="fin-kpi" data-bs-toggle="tooltip" title="صافي الحركة = الدائن - المدين؛ يعطي اتجاه السيولة الدفترية.">@endif<div class="text-muted">Net Movement</div><div class="fw-bold fs-4">@format_currency(($totals->total_credit ?? 0)-($totals->total_debit ?? 0))</div>@if($canTrialBalance)</a>@else</div>@endif</div>
    </div>

    <div class="row g-3 mb-4">
        <div class="col-md-3">@if($canBalanceSheet)<a href="{{ route('balance-sheet', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي الأصول حسب دليل الحسابات خلال الفترة.">@else<div class="fin-kpi">@endif<div class="text-muted">@lang('accounting::lang.assets')</div><div class="fw-bold fs-5">@format_currency($assets_total ?? 0)</div>@if($canBalanceSheet)</a>@else</div>@endif</div>
        <div class="col-md-3">@if($canBalanceSheet)<a href="{{ route('balance-sheet', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي الخصوم ضمن الفترة المحددة.">@else<div class="fin-kpi">@endif<div class="text-muted">@lang('accounting::lang.liabilities')</div><div class="fw-bold fs-5">@format_currency($liabilities_total ?? 0)</div>@if($canBalanceSheet)</a>@else</div>@endif</div>
        <div class="col-md-3">@if($canBalanceSheet)<a href="{{ route('balance-sheet', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي حقوق الملكية المحسوبة من القيود الحالية.">@else<div class="fin-kpi">@endif<div class="text-muted">@lang('accounting::lang.equity')</div><div class="fw-bold fs-5">@format_currency($equity_total ?? 0)</div>@if($canBalanceSheet)</a>@else</div>@endif</div>
        <div class="col-md-3">@if($canBalanceSheet)<a href="{{ route('balance-sheet', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="فرق معادلة الميزانية: يجب أن يكون قريباً من الصفر.">@else<div class="fin-kpi">@endif<div class="text-muted">A=L+E Diff</div><div class="fw-bold fs-5">@format_currency(abs(($assets_total ?? 0) - (($liabilities_total ?? 0)+($equity_total ?? 0))))</div>@if($canBalanceSheet)</a>@else</div>@endif</div>
    </div>

    <div class="row g-3 mb-4">
        <div class="col-md-4">@if($canReceivablesAging)<a href="{{ route('account-receivable-ageing-report', $ageingQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي الذمم المدينة المستحقة حسب تقرير الأعمار.">@else<div class="fin-kpi">@endif<div class="text-muted">Receivables Due</div><div class="fw-bold fs-5">@format_currency($receivables_due ?? 0)</div>@if($canReceivablesAging)</a>@else</div>@endif</div>
        <div class="col-md-4">@if($canPayablesAge)<a href="{{ route('account-payable-ageing-report', $ageingQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="إجمالي الذمم الدائنة المستحقة حسب تقرير الأعمار.">@else<div class="fin-kpi">@endif<div class="text-muted">Payables Due</div><div class="fw-bold fs-5">@format_currency($payables_due ?? 0)</div>@if($canPayablesAge)</a>@else</div>@endif</div>
        <div class="col-md-4">@if($canJournalLedger)<a href="{{ route('journal-report', $reportQuery) }}" class="fin-kpi fin-kpi-link text-decoration-none d-block" data-bs-toggle="tooltip" title="عدد القيود التي لا يتساوى فيها المدين مع الدائن.">@else<div class="fin-kpi">@endif<div class="text-muted">Unbalanced Journals</div><div class="fw-bold fs-5">{{ $unbalanced_journal_entries ?? 0 }}</div>@if($canJournalLedger)</a>@else</div>@endif</div>
    </div>
