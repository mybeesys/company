@php
    ...
    $filterKeys = $allergenFilterKeyIcons ?? $defaultAllergenFilterMap;
@endphp

{{-- يُضمَّن داخل مودال مسببات الحساسية؛ التصفية تُخفي الأصناف التي تحتوي المسبب عند التفعيل --}}
<div class="menu-allergen-filter mt-2 mb-1 px-1" id="menuAllergenFilter">
    @if (count($filterKeys) > 0)
        <div class="d-flex align-items-center justify-content-between gap-2 flex-wrap mb-2">
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <span class="menu-allergen-filter-label small fw-semibold text-muted mb-0">
                    @lang('reservation::lang.menu_allergen_avoid_hint')
                </span>
                <span
                    class="badge rounded-pill bg-secondary menu-allergen-filter-badge d-none"
                    id="allergenFilterActiveBadge"
                    aria-hidden="true"
                >0</span>
            </div>
            <button
                type="button"
                class="btn btn-link btn-sm p-0 menu-allergen-filter-clear d-none"
                id="allergenFilterClear"
                aria-controls="menuAllergenFilter"
            >
                @lang('reservation::lang.menu_allergen_clear')
            </button>
        </div>
        <div
            class="menu-allergen-filter-chips custom-scroll d-flex flex-wrap gap-2 pb-1"
            role="group"
            aria-label="@lang('reservation::lang.menu_allergen_avoid_a11y')"
        >
    ...
    @else
        <p class="small text-muted mb-0">@lang('reservation::lang.menu_allergen_filter_empty')</p>
    @endif
</div>
