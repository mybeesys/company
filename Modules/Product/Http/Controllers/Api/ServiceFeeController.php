...
class ServiceFeeController extends Controller
{
    ...
    public function serviceFeeApplicationTypes()
    {
        ...
        return response()->json([
            'data' => array_map(function($item) {
                $newItem["id"] = $item["value"];
                $newItem["name_ar"] = Lang::get('messages.service_fee_app_type_'.$item["name"], [], 'ar'); ;
                $newItem["name_en"] = Lang::get('messages.service_fee_app_type_'.$item["name"], [], 'en'); ;
                return $newItem;
            }, $result)
        ]);
    }
    ...
    public function serviceFeeCalculationMethods()
    {
        ...
        return response()->json([
            'data' => array_map(function($item) {
                $newItem["id"] = $item["value"];
                $newItem["name_ar"] = Lang::get('messages.service_fee_calc_method_'.$item["name"], [], 'ar'); ;
                $newItem["name_en"] = Lang::get('messages.service_fee_calc_method_'.$item["name"], [], 'en'); ;
                return $newItem;
            }, $result)
        ]);
    }
    ...
    public function serviceFeeAutoApplyTypes()
    {
        ...
        return response()->json([
            'data' => array_map(function($item) {
                $newItem["id"] = $item["value"];
                $newItem["name_ar"] = Lang::get('messages.'.$item["name"], [], 'ar'); ;
                $newItem["name_en"] = Lang::get('messages.'.$item["name"], [], 'en'); ;
                return $newItem;
            }, $result)
        ]);
    }
