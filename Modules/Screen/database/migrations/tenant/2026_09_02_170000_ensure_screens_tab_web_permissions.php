...
return new class extends Migration
{
    ...
    {
        $rows = [];
        foreach (['promos' => 'المواد الإعلانية', 'playlists' => 'قوائم التشغيل', 'devices' => 'الأجهزة'] as $entity => $nameAr) {
            foreach (ScreenPermissions::crud($entity) as $name) {
    ...
    public function up(): void
    {
        ...
        foreach (['show', 'create', 'update', 'delete'] as $action) {
            $this->copyAssignments($table, $rolePivot, $modelPivot, ScreenPermissions::crud('main')[$action], [
                $ids[ScreenPermissions::crud('promos')[$action]] ?? null,
                $ids[ScreenPermissions::crud('playlists')[$action]] ?? null,
                $ids[ScreenPermissions::crud('devices')[$action]] ?? null,
            ]);
        }
