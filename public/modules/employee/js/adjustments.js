function initializeSelect2(
    ...
    `);
        ...
        templateSelection: function (option) {
            if (option.addNew) {
                return $(
                    `<input type="text" class="select2-add-new-input" placeholder="${
                        lang === "ar"
                            ? "اكتب الخيار الجديد"
                            : "Type new option..."
                    }"/>`
...
function adjustmentRepeater(
    ...
    $(`#${type}_repeater`).repeater({
        ...
        ready: function () {
            ...
            $repeater
                .find(`select[name^="${type}_repeater"][name$="[amount_type]"]`)
                .select2({
        ...
        show: function () {
            ...
            let adjustment_type = $this.find(
                `select[name^="${type}_repeater"][name$="[adjustment_type]"]`
            );

            const customOptions = new Map();

            adjustment_type.find("option").remove();

            customOptions.forEach(function (option) {
                adjustment_type.append(new Option(option.text, option.id));
            });
