...
const ProductBarcode = ({ translations, dir }) => {
    ...
    const onChange = (key, value) => {
        let r = { ...currentObject };
        r[key] = value;
       //ss console.log("Data", currentObject);
        setcurrentObject({ ...r });
        return { message: "Done" };
    };
    ...
    const getProductPrice = (row) => {
        if (!!row.priceTier)
            return currentObject.pricingType.value == 0
                ? row.priceTier.data.price
                : row.priceTier.data.price_with_tax;
        else
            return currentObject.pricingType.value == 0
                ? row.product.data.price
                : row.product.data.price_with_tax;
    };
                        ...
                        <>
                            ...
                            <div class="d-flex  align-items-center pt-3">
                                ...
                                <div class="form-check form-switch">
                                    <InputSwitch
                                        checked={
                                            !!currentObject.expiryDate
                                                ? !!currentObject.expiryDate
                                                : false
                                        }
                                        onChange={(e) =>
                                            onChange("expiryDate", e.value)
                                        }
                                    />
                                </div>
                            </div>
                            <div class="d-flex  align-items-center pt-3">
                                <label
                                    ...
                                    style={{ width: "120px" }}
                                >
                                    {translations.sellDate}
                                </label>
                                <div class="form-check form-switch">
                                    <InputSwitch
                                        checked={
                                            !!currentObject.sellDate
                                                ? !!currentObject.sellDate
                                                : false
                                        }
                                        onChange={(e) =>
                                            onChange("sellDate", e.value)
                                        }
                                    />
                                </div>
                            </div>
                                ...
                                Array.from({ length: product.count }).map(
                                    (_, index) => (
                                        ...
                                        >
                                            ...
                                            {currentObject.expiryDate &&
                                            product.expiryDate ? (
                                                <div>{`${translations.expiryDate} ${product.expiryDate}`}</div>
                                            ) : (
                                                <></>
                                            )}
                                            {currentObject.sellDate &&
                                            product.sellDate ? (
                                                <div>{`${translations.sellDate} ${product.sellDate}`}</div>
                                            ) : (
                                                <></>
                                            )}
                                            <Barcode
                                                value={
                                                    product.product.data.barcode
                                                }
                                                format="CODE128"
                                                width={1.5}
                                                height={50}
