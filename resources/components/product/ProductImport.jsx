...
const ProductImport = ({ translations, dir }) => {
        ...
        try {
            ...
            if (response.data.message == "Done") {
                ...
                Swal.fire({
                    show: showAlert,
                    title: "Error",
                    html: `<div>${translations.products} - ${
                        translations.errorInImport
                    }</div>${getErrorMessage(response.data.errors)}`,
                    icon: "error",
                    timer: 4000,
                    showCancelButton: false,
                    showConfirmButton: false,
    ...
    const getErrorMessage = (data) => {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            res += `<div>${translations.product}: ${getRowName(
                element.row
            )}</div>`;
            res += `<div>${element.message.message
                .split("_")
                .map((m) => `${translations[m]} `)}: ${element.message.data.map(
                (m) => `${!!translations[m] ? translations[m] : m}, `
            )}</div>`;
        }
        return res;
    };
            ...
            <div class="card-header border-0 pt-5">
                ...
                <div class="card-toolbar">
                    <div class="d-flex align-items-center gap-2 gap-lg-3">
                        <a href={templateUrl} style={{ width: "10rem" }}>
