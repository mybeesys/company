...
const ServiceFeeDetail = ({ dir, translations }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const lFeeTypes = res2.data.map((e) => {
                return {
                    name: translations[`service_fee_type_${e.name}`],
                    value: e.value,
                };
            });
            const res3 = await axios.get("/serviceFeeAppTypeValues");
            const lAppTypes = res3.data.map((e) => {
                return {
                    name: translations[`service_fee_app_type_${e.name}`],
                    value: e.value,
                };
            });
            const res4 = await axios.get("/serviceFeeCalcMetheodValues");
            const lCalcMethods = res4.data.map((e) => {
                return {
                    name: translations[`service_fee_calc_method_${e.name}`],
                    value: e.value,
                };
            });
            const res5 = await axios.get("/serviceFeeAutoApplyValues");
            const lAutoApplyTypes = res5.data.map((e) => {
                return { name: translations[e.name], value: e.value };
            });
            const res6 = await axios.get("/paymentMethods");
            const lCreditCardTypes = res6.data.map((e) => {
                return {
                    name: dir === "rtl" ? e.name_ar : e.name_en,
                    value: e.id,
                };
            });
            const res7 = await axios.get("/paymentCards");
            const lPaymentCards = res7.data.map((e) => {
                return { label: getName(e.name_en, e.name_ar), value: e.id };
            });
            const res8 = await axios.get("/diningTypes");
            const lDiningTypes = res8.data.map((e) => {
                return { label: getName(e.name_en, e.name_ar), value: e.id };
            });
    ...
    const toSelectValues = (values, options, key) => {
        return values.map((x) => {
            return {
                label: !!options.filter((y) => y.value == x[key]).length
                    ? options.filter((y) => y.value == x[key])[0].label
                    : null,
                value: x[key],
            };
        });
                                ...
                                <div class="tab-content">
                                    ...
                                    >
                                        <ServiceFeeAutoApply
                                            translations={translations}
                                            currentObject={currentObject}
                                            serviceFeeCards={toSelectValues(
                                                currentObject["cards"],
                                                paymentCards,
                                                "payment_card_id"
                                            )}
                                            serviceFeediningTypes={toSelectValues(
                                                currentObject["diningTypes"],
                                                diningTypes,
                                                "dining_type_id"
                                            )}
                                            onBasicChange={onBasicChange}
