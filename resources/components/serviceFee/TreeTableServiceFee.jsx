...
const defaultValue = {active : 1, service_fee_type: 0, application_type: 0, calculation_method:0, taxable: 0}
const TreeTableServiceFee = ({ urlList, rootElement, translations, appTypes, feeTypes, calcMethods}) => {
    ...
    const editRow = (data, key) => {
        window.location.href =  '/serviceFee' + '/'+data.id + '/edit';
        // setCurrentKey(key);
        // setEditingRow({ ...data });
    }

    const openAddServiceFee = () => {
        window.location.href = 'serviceFee/create'
    }
        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bold fs-3 mb-1">{translations.serviceFees}</span>
                </h3>
            ...
            <form  id="treeForm" noValidate validated={true} class="needs-validation" onSubmit={handleSubmit}>
                <TreeTable  value={nodes}  tableStyle={{ minWidth: '50rem' }} className={"custom-tree-table"}>
                    ...
                    <Column header={translations.amount} style={{ width: '6%' }}  body={(node) => (renderDecimalCell(node, 'amount'))} sortable> </Column>
                    <Column header={translations.type} style={{ width: '10%' }}  body={(node) => (renderDropDownCell(node, 'service_fee_type', false, feeTypes))} sortable> </Column>
                    <Column header={translations.application_type} style={{ width: '11%' }}  body={(node) => (renderDropDownCell(node, 'application_type', false, appTypes))} sortable> </Column>
                    <Column header={translations.taxable} style={{ width: '6%' }}  body={(node) => (renderCheckCell(node, 'taxable'))} sortable> </Column>
                    <Column header={translations.calculation_method} style={{ width: '10%' }}  body={(node) => (renderDropDownCell(node, 'calculation_method', false, calcMethods))} sortable> </Column>
...
export default TreeTableServiceFee;