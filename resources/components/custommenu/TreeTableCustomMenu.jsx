...
const TreeTableCustomMenu = ({
    ...
    const cancelEdit = (key) => {
        if (!!!editingRow.id || editingRow.id == 0) {
            ...
            for (let index = 0; index < currentNodes.length; index++) {
                ...
                if (node.key == key) {
                    if (!!parentNode) parentNode.children.splice(index, 1);
    ...
    const renderMultipleDropDownCell = (node, key, autoFocus, options = []) => {
        if (!Array.isArray(options) || options.length === 0) {
            return <span>لا توجد خيارات متاحة</span>;
        }

        const selectedValues = Array.isArray(node.data[key])
            ? node.data[key].map(String)
            : [];
        ...
        return node.key == currentKey ? (
            <Select
                ...
                onChange={(selected) => {
                    const newValues = selected.map((opt) => String(opt.value));
                    node.data[key] = newValues;
                    handleEditorChange(newValues, key);
                }}
                autoFocus={!!autoFocus}
    ...
    const renderModeStationCell = (node, key, autoFocus, modesStations) => {
        ...
        const valKey =
            !!application_type && application_type == "3"
                ? "station_id"
                : "mode";
        return renderMultipleDropDownCell(node, valKey, autoFocus, options);
    };
            ...
            <div class="card-body">
                ...
                >
                    ...
                    >
                        ...
                        ></Column>
                        <Column
                            header={translations.application_type}
                            style={{ width: "15%" }}
                            body={(node) =>
                                renderDropDownCell(
                                    node,
                        ...
                        ></Column>
                        <Column
                            header={translations.mode_station_name}
                            style={{ width: "15%" }}
                            body={(node) =>
