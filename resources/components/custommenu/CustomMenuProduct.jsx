...
const CustomMenuProduct = ({ translations, customMenuProducts, onProductSelectionChange, dir }) => {
    ...
    const checkSelected = (product, sKeys) => {
        ...
        if(product.data.type == 'product' &&
            customMenuProducts.filter(x=> x.product_id == product.data.id).length >0){
            ...
            if(!!product.children && product.children.length > 0){
                ...
                for (let index1 = 0; index1 < product.children.length; index1++) {
    ...
    const fillSelectedProducts = (products, keys,selectedProductIds) => {
        for (let index = 0; index < products.length; index++) {
            const product = products[index];
            if(Object.keys(keys).filter(x => x == product.key).length > 0 && 
                product.data.type == "product" &&
                keys[product.key].checked)
                selectedProductIds.push(product.data.id);
            if(!!product.children)
                fillSelectedProducts(product.children, keys,selectedProductIds);
        }
    }
                    ...
                    <div class="trending__product">
                        ...
                        <div class="container">
                            ...
                            <TreeTable 
                                ...
                                onSelectionChange={(e) => onSelectedProductChange(e.value)}
                                value={products} 
