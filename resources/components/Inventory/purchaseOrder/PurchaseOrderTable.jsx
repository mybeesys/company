...
const PurchaseOrderTable = ({ dir, translations }) => {
  ...
  const changeStatus = (data, status, afterExecute)=>{    
    axios.post('poStatusUpdate', {id: data.id, po_status: status})
        .then((resp)=>{
          data["po_status"] = resp.data.po_status;
          data["po_status_name"] = resp.data.po_status_name;
          Swal.fire({
            show: showAlert,
            title: `${translations['purchaseOrder']}: ${data.no} ${translations[resp.data.po_status_name]}`,
  ...
  const dropdownCell = (data, key, editMode, editable, refreshTree) => {
    ...
    if(data.po_status != 4 && data.po_status!= 3)
    ...
    <div>
      ...
      <TreeTableComponent
        ...
        cols={[
          ...
          {key : "po_date", autoFocus: true, type :"Date", width:'15%'},
          {key : "po_status", autoFocus: true, type :"Date", width:'15%',
