...
const UnitTransferIngredient = ({translations , unitTransfer, unitTree, parentHandle ,dir}) => {
...
const renderDropDownCell = (node, key, autoFocus) => {
    ...
    return (
      ...
      />
          ...
          :
          <span>{!!unitTree.find(x => x.value == node[key]) ? unitTree.find(x => x.value == node[key]).label : ''}</span>);
}
