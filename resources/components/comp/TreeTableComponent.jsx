...
const TreeTableComponent = ({
    ...
    const renderDateCell = (node, col, index) => {
        ...
        return node.key == currentKey ? (
            <Calendar
                ...
                onChange={(e) => {
                    let value = !!e.value
                        ? `${e.value.getFullYear()}-${(e.value.getMonth() + 1)
                              .toString()
                              .padStart(2, "0")}-${e.value
                              .getDate()
                              .toString()
                              .padStart(2, "0")}`
                        : null;
            ...
            <span>
                {!!node.data[col.key]
                    ? `${new Date(node.data[col.key]).getFullYear()}-${(
                          new Date(node.data[col.key]).getMonth() + 1
                      )
                          .toString()
                          .padStart(2, "0")}-${new Date(node.data[col.key])
                          .getDate()
                          .toString()
                          .padStart(2, "0")}`
                    : null}
    ...
    const renderCheckCell = (node, col, index) => {
        return node.key == currentKey ? (
            <div>
                <input
                    type="checkbox"
                    checked={editingRow[col.key] == 1 ? true : false}
                    class="form-check-input"
                    data-kt-check={node.data[col.key]}
    ...
    const renderAsyncDropDownCell = (node, col, index) => {
    ...
    const clearAddRow = (nodesData) => {
        for (let index = 0; index < nodesData.length; index++) {
            if (!!nodesData[index].children)
                nodesData[index].children = clearAddRow(
                    nodesData[index].children
                );
            if (
                !!nodesData[index].data.empty &&
                nodesData[index].data.empty == "Y"
            ) {
                    ...
                    >
                        {cols.map((col, index) => (
                            <Column
                                ...
                                body={(node) => renderCell(node, col, index)}
