...
const TreeTableEditorUnit = ({
    ...
    const urlPar = urlParams.get("type");

    const togglePopup = (id1, id2) => {
    ...
    const fetchData = async (id1, id2) => {
        try {
            const url = `${window.location.origin}/transfer/${id1}/partial-deliveries/${id2}`;
    ...
    const updateBasicData = (nodes) => {
        const response = onUpdate(
            nodes.filter((x) => x.data.empty == "Y").length > 0
                ? nodes.slice(0, nodes.length - 1).map((x) => {
    ...
    const handleEditorChange = (value, key, rowKey, onChangeValue) => {
        ...
        if (!!onChangeValue) {
            onChangeValue(
                ...
                value,
                rowKey,
                (nodes, updateOriginalData) => {
                    setNodes([...nodes]);
                    if (!!updateOriginalData) updateBasicData(nodes);
                }
            );
        }
    ...
    const renderDateCell = (node, col, firstCell) => {
        ...
        if (!!node.data.empty) {
            ...
            return !!col.editable ? (
                <Calendar
                    class="col-12"
                    value={toDate(node.data[col.key], "D")}
                    onChange={(e) =>
                        handleEditorChange(
                            !!e.value
                                ? `${e.value.getFullYear()}-${(
        ...
        if (!!node.data.empty) {
            ...
            return !!col.editable ? (
                <div>
                    <input
                        type="checkbox"
                        checked={node.data[col.key] == 1 ? true : false}
    ...
    const renderAsyncDropDownCell = (node, col, firstCell) => {
        ...
        let url = !!col.relatedTo
            ? `${col.searchUrl}?${col.relatedTo.key}=${
                  get(node.data, col.relatedTo.relatedKey)
                      ? get(node.data, col.relatedTo.relatedKey)
                      : -1
              }`
            : col.searchUrl;
    ...
    const renderMultiDropDownCell = (node, col, firstCell) => {
        ...
        const val = col.options.filter((x) =>
            node.data[col.key].includes(x.value)
        );
...
export default TreeTableEditorUnit;
