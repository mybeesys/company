...
const ModifierComponent = ({ translations, dir }) => {
    ...
    const [unitTransfer, setUnitTransfers] = useState(
        !!modifier.unitTransfer ? modifier.unitTransfer : []
    );
        ...
        try {
            ...
            if (!!modifierUnit) {
                if (!!!modifierUnit.id)
                    transfer.push({
                        id: 0,
                        unit1: modifierUnit.unit1,
                        unit2: -100,
                        transfer: -100,
                        primary: -100,
                    });
                else transfer.push(modifierUnit); //{ id: 0 , unit1: productUnit , unit2: -100 , transfer: -100 , primary :-100});
            }
    ...
    const getModifierLOVs = async () => {
        ...
        const taxes = response.data.taxes.map((e) => {
            return {
                label: getRowName(e, dir),
                value: e.id,
                default: e.default,
            };
        });
        const modifierClasses = response.data.modifierClasses.map((e) => {
            return { label: getRowName(e, dir), value: e.id };
        });
