...
const TreeTableDiscount = ({ urlList, rootElement, translations, discountTypes, discountFunctions}) => {
    ...
    const editRow = (data, key) => {
        window.location.href =  '/discount' + '/'+data.id + '/edit';
        // setCurrentKey(key);
        // setEditingRow({ ...data });
    }

    const openAddServiceFee = () => {
        window.location.href = 'discount/create'
    }
        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <form  id="treeForm" noValidate validated={true} class="needs-validation" onSubmit={handleSubmit}>
                <TreeTable  value={nodes}  tableStyle={{ minWidth: '50rem' }} className={"custom-tree-table"}>
                    ...
                    <Column header={translations.discountFunction} style={{ width: '6%' }}  body={(node) => (renderDropDownCell(node, 'function_id', false, discountFunctions))} sortable> </Column>
                    <Column header={translations.type} style={{ width: '10%' }}  body={(node) => (renderDropDownCell(node, 'discount_type', false, discountTypes))} sortable> </Column>
                    <Column header={translations.amount} style={{ width: '11%' }}  body={(node) => (renderDecimalCell(node, 'amount'))} sortable> </Column>
                    <Column header={translations.autoApply} style={{ width: '6%' }}  body={(node) => (renderCheckCell(node, 'auto_apply'))} sortable> </Column>
                    <Column header={translations.itemLevel} style={{ width: '6%' }}  body={(node) => (renderCheckCell(node, 'item_level'))} sortable> </Column>
...
export default TreeTableDiscount;